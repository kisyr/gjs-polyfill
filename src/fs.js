...
export function writeFileSync(path, data, options = { encoding: null }) {
	const file = Gio.File.new_for_path(path);

	const [ ok, tag ] = file.replace_contents(
		data,
		null,
		false,
...
export function writeFile(path, data, options = { encoding: null }) {
	return new Promise(resolve => {
